...
impl MontyObject {
    ...
    ///
    /// Requires a `&mut VM` because:
    /// - `from_value` needs `&VM` to call `py_repr` for fallback representations
    /// - `drop_with_heap` needs `&mut Heap` to decrement reference counts
    ...
    fn from_value_inner(object: &Value, vm: &VM<'_, '_, impl ResourceTracker>, visited: &mut AHashSet<HeapId>) -> Self {
        ...
        Self::from_value_inner_impl(object, vm, visited)
    }
